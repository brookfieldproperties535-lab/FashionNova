      ...
      <div class="row justify-content-center">
        <div class="col-sm-11 col-md-9 col-lg-7">
          ...
          <div class="text-center mb-5"
            ...
            <h5 class="text-uppercase ls fw-black font-secondary">Frequently Asked Questions</h5>
          </div>

          {{-- Accordion --}}
          <div class="accordion border rounded" id="faqAccordion"
            ...
            @foreach ($faqs as $i => $faq)
              <div class="accordion-item {{ $loop->first ? '' : 'border-top' }}" style="border: none;">
                <h2 class="accordion-header" id="faq-heading-{{ $i }}">
                  <button
                    class="accordion-button {{ $loop->first ? '' : 'collapsed' }} font-secondary fw-bold fs-10 text-uppercase ls"
                    type="button"
                    data-bs-toggle="collapse"
                    data-bs-target="#faq-collapse-{{ $i }}"
                    aria-expanded="{{ $loop->first ? 'true' : 'false' }}"
                    aria-controls="faq-collapse-{{ $i }}"
                    style="background:transparent;color:inherit;box-shadow:none;">
                    <span class="badge rounded-pill bg-dark text-white font-secondary me-2 px-2" style="font-size:9px;">{{ $faq['cat'] }}</span>
                    {{ $faq['q'] }}
                  </button>
                </h2>
                <div id="faq-collapse-{{ $i }}"
                  ...
                  <div class="accordion-body font-secondary fs-10 text-body-tertiary pt-0">
                    {{ $faq['a'] }}
                  </div>
                </div>
              </div>
          ...
          <div class="border rounded p-4 text-center mt-4"
            ...
            <h6 class="fw-bold mb-2">Still Have Questions?</h6>
            <p class="font-secondary fs-10 text-body-tertiary mb-3">
              Our support team is available 24/7 to help you.
            </p>
            <a href="{{ route('user.support') }}"
              class="btn btn-dark px-5 text-uppercase ls fw-black font-secondary">
              Contact Support
            </a>
          </div>
